...
### 2.3.2. Prepare for Training Sets
```{r}
set.seed(1)
...
### 2.3.3. TrainData Generating Function
```{r}
...
for (i in 1:RandomTimes){
    ...
    NewCols <- match(c("Date","Location","RISK_MM"),names(TrainData))
...
```{r cache=TRUE, results='hide'}
AllDTPred <- matrix(nrow = nrow(TestData), ncol = length(AllDTree))
AllDTPredAccuracy <- matrix(nrow = length(AllDTree), ncol = 1)
for (i in 1:length(AllDTree)){
    AllDTPred[,i] <- (predict(AllDTree[[i]],TestData,type="class") == "Yes")
    AllDTPredAccuracy[i] <- mean(AllDTPred[,i]==(TestData$RainTomorrow=="Yes"))
    print(paste0(as.character(i),"/",as.character(RandomTimes),"-Tree Predict"))
}
```
...
From the histogram, the prediction accuracy of all these trees are between 71% and 75%. The majority is below 73%. (Note that the testing dataset has balanced samples.)

### 3.5.3. Bagging Without Selecting Indicators
```{r}
BaggingPred <- (rowMeans(AllDTPred) > 0.5)
BaggingPredAccuracy <- mean(BaggingPred == (TestData$RainTomorrow=="Yes"))
```
...
From the graph above, the red line, representing the prediction accuracy after bagging, locates on the right side of the histogram, meaning bagging without selecting indicators can improve prediction accuracy.

### 3.5.4. Random Forest - Prediction
```{r}
TreeBagPred <- 
