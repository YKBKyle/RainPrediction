...
Library(leaps)
head(Data)
RainTommorrow = regsubsets(YesRain~.-RainTommorrow, data=Data)
Data.train=Data[TrainData,]
Data.test=Data[TestData,]
TrainData <- GetTrainData(1)
log.reg.fit=glm(YesRain01~.,family=binominal,data=Data.train)
summary(log.reg.fit)
YesRain01.probs=predict(log.reg.fit,Data.test,type="response")
YesRain01.pred = ifelse(YesRain01,probs>=0.5,1,0)
mean(YesRain01.pred==YesRain01.test)
```
...
GetTreePredict <- function(AllDTree,TestData){
    ...
    for (i in 1:length(AllDTree)){
        ...
        print(paste0(as.character(i),"/",as.character(RandomTimes),"-Predict"))
    }
    TreePredict
}
...
Prediction:
```{r cache=TRUE, results='hide'}
AllDTPred_IndLoc <- matrix(nrow = nrow(TestData), ncol = length(AllDTree_IndLoc))
AllDTPredAccuracy_IndLoc <- matrix(nrow = length(AllDTree_IndLoc), ncol = 1)
for (i in 1:length(AllDTree_IndLoc)){
    AllDTPred_IndLoc[,i] <- (predict(AllDTree_IndLoc[[i]],ChangeLocation2Vector(TestData),type="class") == "Yes")
    AllDTPredAccuracy[i] <- mean(AllDTPred[,i]==TestRainTomorrow)
