...
AllDTPred <- as.data.frame(AllDTPred)
AllDTPredAccuracy <- matrix(nrow = length(AllDTree), ncol = 1)
for (i in 1:nrow(DTPredAccuracy)){
    AllDTPred[,i] <- predict(AllDTree[[i]],TestData,type="class")
    AllDTPredAccuracy[i] <- mean(AllDTPred[,i]==TestData$RainTomorrow)
